...
function getRedeemedItems() {
  try { return JSON.parse(localStorage.getItem('redeemedItems')) || []; }
  catch(e) { return []; }
}
...
function markItemRedeemed(elId, itemName) {
  const el = document.getElementById(elId);
  if (!el) return;
  el.classList.add('redeemed');
  el.onclick = null;
  const costEl = el.querySelector('.mkt-item-cost');
  if (costEl) costEl.textContent = 'Redeemed ✓';
}

function initMarketplace() {
  const redeemed = getRedeemedItems();
  const idMap = {
  ...
  redeemed.forEach(name => {
    if (idMap[name]) markItemRedeemed(idMap[name], name);
  });
...
function redeemItem(elId, itemName, cost) {
  ...
  if (redeemed.includes(itemName)) {
    showMktToast('Already redeemed: ' + itemName);
    return;
  }
